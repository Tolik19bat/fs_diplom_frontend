...
export default class PosterList {
  ...
  bindToDom() {
    ...
    this.mainEl.addEventListener(
      "updatePosterList",
      this.onUpdatePosterList.bind(this)
    ); // Регестриуем событие
  ...
  async onUpdatePosterList() {
    const response = await Fetch.send("GET", "movie"); // Отправляем асинхронный GET-запрос на сервер для получения списка фильмов
    this.getPosters(response); // Вызываем метод `getPosters`, передавая полученный список фильмов (`response`), чтобы обработать данные о постерах
    this.renderPosters(); // Вызываем метод `renderPosters`, который отрисовывает постеры фильмов на странице
