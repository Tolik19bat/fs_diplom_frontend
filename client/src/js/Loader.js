...
export default class Loader { // Создаем класс `Loader` для управления индикатором загрузки
  ...
  static init() { // Метод инициализации лоадера
  ...
  static startLoader() { // Метод для запуска лоадера
    if (Loader.start) { // Проверяем, не запущен ли уже лоадер
  ...
  static stopLoader() { // Метод для остановки лоадера
    ...
    Loader.timeId = setTimeout(() => { // Запускаем таймер перед скрытием лоадера
  ...
  static renderLoader() { // Метод для отображения лоадера
    Loader.loaderEl.style.display = ""; // Делаем элемент видимым
  }

  static haideLoader() { // Метод для скрытия лоадера (в названии опечатка, должно быть `hideLoader`)
